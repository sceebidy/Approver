...
import { LayoutGrid, ClipboardList, FileText, CheckCircle2, Wallet, User } from "lucide-react";
...
function NavItem({ icon: Icon, label, href }: { icon: LucideIcon; label: string; href: string }) {
  ...
  return (
    <Link
      href={href}
      className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-md text-[13.5px] transition-colors ${active ? "bg-[#1F3A5F] text-white font-medium" : "text-[#4B5563] hover:bg-[#F1F3F6]"
        }`}
    >
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-56 shrink-0 border-r border-[#E3E6EA] bg-white flex flex-col h-screen">
      ...
      <nav className="flex-1 px-2.5 py-3 space-y-0.5 text-sm overflow-y-auto">
